...
export const addToCart = createAsyncThunk(
  ...
  async (product: Omit<CartItem, 'qty'>, { getState, rejectWithValue }) => {
    ...
    const existingItem = state.cart.items.find(
      (p) =>
        p.variantId === product.variantId && !!p.hasCustomization === !!product.hasCustomization
    );
...
const cartSlice = createSlice({
  ...
  reducers: {
    ...
    decrementItem: (
      state,
      action: PayloadAction<{ variantId: number; hasCustomization?: boolean }>
    ) => {
      ...
      const existingItem = state.items.find(
        (item) => item.variantId === variantId && !!item.hasCustomization === !!hasCustomization
      );
    ...
    toggleCustomization: (
      ...
      const targetItemIndex = state.items.findIndex(
        (i) => i.variantId === variantId && i.hasCustomization !== hasCustomization
      );
      ...
      if (targetItemIndex !== -1) {
        ...
        const existingMergeIndex = state.items.findIndex(
          (i, idx) =>
            idx !== targetItemIndex &&
...
export const { setCart, decrementItem, removeItem, clearCart, toggleCustomization } =
  cartSlice.actions;
...
export const selectCartSubtotal = (state: { cart: CartState }) =>
  state.cart.items.reduce((acc, item) => {
    const itemPrice =
      item.price + (item.hasCustomization && item.customizationCost ? item.customizationCost : 0);
